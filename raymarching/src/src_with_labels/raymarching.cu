...
__global__ void kernel_composite_rays_train_forward(
    ...
    const scalar_t * __restrict__ classes,  // <-- add new input for classes
    ...
    scalar_t * depth,
    scalar_t * image,
    scalar_t * semantic_image  // <-- add new output for semantic image
) {
    // parallel per ray
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= N) return;
    ...
    if (num_steps == 0 || offset + num_steps >= M) {
        ...
        for (uint8_t i = 0; i < num_classes; i++) {
            semantic_image[index * num_classes + i] = 0;  // <-- initialize semantic image to 0
        }
        return;
    }
    ...
    scalar_t semantic_image_per_class[20] = {0};  // TODO: Allocate memory according to the number of classes
    ...
    while (step < num_steps) {
        ...
        for (uint8_t i = 0; i < num_classes; i++) {
            semantic_image_per_class[i] += weight * classes[i];
        }
    ...
    image[index * 3 + 2] = b;
    for (uint8_t i = 0; i < num_classes; i++) {
        semantic_image[i] = semantic_image_per_class[i];
    }

}


void composite_rays_train_forward(const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor classes, const at::Tensor deltas, const at::Tensor rays, const uint8_t num_classes, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor weights_sum, at::Tensor depth, at::Tensor image, at::Tensor semantic_image) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    sigmas.scalar_type(), "composite_rays_train_forward", ([&] {
        kernel_composite_rays_train_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), classes.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), num_classes, M, N, T_thresh, weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), semantic_image.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays_train_backward(
    ...
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs, 
    const scalar_t * __restrict__ classes,
    const scalar_t * __restrict__ deltas,
    const int * __restrict__ rays,
    const uint8_t num_classes,
    ...
    grad_image += index * 3;
    grad_semantic_image += index * num_classes;
    weights_sum += index;
    image += index * 3;
    semantic_image += index * num_classes;
    sigmas += offset;
    rgbs += offset * 3;
    classes += offset * num_classes;
    deltas += offset * 2;
    grad_sigmas += offset;
    grad_rgbs += offset * 3;
    grad_classes += offset * num_classes;
...
//     const escalar_t semantic_image_per_class_final[num_classes] = smantic_image;
    ...
    while (step < num_steps) {
        ...
        for (uint8_t i = 0; i < num_classes; i++) {
            grad_classes[i] = grad_semantic_image[i];
        }


        // write grad_sigmas TODO: check how to add grads for classes
...
void composite_rays_train_backward(const at::Tensor grad_weights_sum, const at::Tensor grad_image, const at::Tensor grad_semantic_image, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor classes, const at::Tensor deltas, const at::Tensor rays, const uint8_t num_classes, const at::Tensor weights_sum, const at::Tensor image, const at::Tensor semantic_image, const uint32_t M, const uint32_t N, const float T_thresh, at::Tensor grad_sigmas, at::Tensor grad_rgbs, at::Tensor grad_classes) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_train_backward", ([&] {
        kernel_composite_rays_train_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights_sum.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(), grad_semantic_image.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), classes.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), num_classes, weights_sum.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), semantic_image.data_ptr<scalar_t>(), M, N, T_thresh, grad_sigmas.data_ptr<scalar_t>(), grad_rgbs.data_ptr<scalar_t>(), grad_classes.data_ptr<scalar_t>());
    }));
}
...
__global__ void kernel_composite_rays(
    ...
    scalar_t semantic_image_per_class[20] = {0};  // TODO: Allocate memory according to the number of classes
    for (uint8_t i = 0; i < num_classes; i++) {
        semantic_image_per_class[i] = classes[i];
    }
...
void composite_rays(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, at::Tensor rays_alive, at::Tensor rays_t, const uint8_t num_classes, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor classes, const at::Tensor deltas, at::Tensor weights, at::Tensor depth, at::Tensor image, at::Tensor semantic_image) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays", ([&] {
        kernel_composite_rays<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), num_classes, sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), classes.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), semantic_image.data_ptr<scalar_t>());
    }));
}